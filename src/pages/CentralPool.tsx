...
export default function CentralPool() {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="py-24 text-center text-sm text-[color:var(--nx-muted)]">Loading pool…</div>
      ) : leads.length === 0 ? (
        <div className="rounded-lg border border-dashed border-[#CAC8C7] p-16 text-center">
          ...
          <p className="mt-1 text-sm text-[color:var(--nx-muted)]">
          ...
          {leads.map(l => (
            <div key={l.id} className="rounded-lg border border-[#DFDDDD] p-4 transition-shadow hover:shadow-md">
              <div className="mb-1 flex items-center justify-between">
                ...
                <span className="text-[11px] text-[color:var(--nx-muted)]">{formatDateTime(l.submittedAtUtc)}</span>
              </div>
              <div className="font-bold text-[#333333]">{l.name}</div>
              <div className="mb-1 flex items-center gap-2 text-xs text-[color:var(--nx-muted)]">
              ...
              <div className="mb-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[#333333]">
                ...
                <span className="text-[color:var(--nx-muted)]">{l.cta ?? ""}</span>
                {l.reportTitle && <span className="w-full truncate text-[color:var(--nx-muted)]">{l.reportTitle}</span>}
              </div>
              <div className="flex items-center gap-2">
                ...
                {!can("ownLeads") && !can("reassign") && (
                  <span className="text-xs italic text-[color:var(--nx-muted)]">View only — your role cannot handle leads</span>
                )}
              </div>
            </div>
          ))}
